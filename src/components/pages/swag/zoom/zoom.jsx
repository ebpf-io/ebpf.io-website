    ...
    <div className="container flex items-center justify-between space-x-24 lg:block lg:space-x-0">
      ...
      <div className="max-w-[488px] lg:mt-12 lg:max-w-full lg:text-center">
        <div className="flex items-center space-x-5 lg:justify-center">
          <ZoomIcon className="h-11 w-11" />
          <h2 className="heading-8xl font-bold leading-dense">{data.title}</h2>
        </div>
        <p className="mt-4 text-xl lg:mx-auto">{data.description}</p>

        <a
          className="hover:bg-black-hover focus-visible:ring-outline relative mt-5 inline-flex h-[52px] items-center justify-center whitespace-nowrap rounded-md bg-black px-12 font-extrabold leading-none text-white transition-colors transition-opacity duration-200 focus:outline-none focus-visible:ring-2 disabled:pointer-events-none md:h-[44px] sm:w-full"
          href={data.buttonUrl}
          theme="black"
          download
        >
